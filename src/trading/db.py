...
async def create_trading_tables() -> None:
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("""
            ...
            ADD COLUMN IF NOT EXISTS stop_loss_price NUMERIC(6,4),
            ADD COLUMN IF NOT EXISTS stop_loss_triggered BOOLEAN DEFAULT FALSE,
            ADD COLUMN IF NOT EXISTS take_profit_order_id TEXT,
            ADD COLUMN IF NOT EXISTS take_profit_price NUMERIC(6,4),
            ADD COLUMN IF NOT EXISTS take_profit_triggered BOOLEAN DEFAULT FALSE
        """)
        await conn.execute("""
            UPDATE bot_trades
