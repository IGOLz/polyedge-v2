...
function formatTimestamp(value, timezoneMode) {
    if (!value)
        return "n/a";
...
function filterAlerts(payload, source) {
    return payload.alerts.filter((alert) => alert.source === source);
}
function buildTileModel(tileId, payload, context) {
    switch (tileId) {
        ...
        case "asset_sol":
        case "asset_xrp": {
            const assetKey = tileId.replace("asset_", "");
            ...
            return {
                ...
                footer: truncate(`15m ${asset.intervals["15m"].observedTicks} • ${formatAge(asset.latestTickAgeSec)}`, 20),
                status: asset.status,
            };
        }
        case "trading_status":
            return {
                label: "Trading",
                value: payload.trading.status === "healthy"
                    ? "LIVE"
        ...
        case "merge_status":
            return {
                label: "Merge Bot",
                value: payload.weatherMerge.status === "healthy"
                    ? "LIVE"
                    : payload.weatherMerge.standDownReason
                        ...
                        : payload.weatherMerge.status === "stale"
                            ? "STALE"
                            : "IDLE",
                footer: truncate(payload.weatherMerge.standDownReason
                    ? payload.weatherMerge.standDownReason
                    : `cand ${payload.weatherMerge.candidateCount}`, 20),
        ...
        case "clone_status":
            return {
                label: "Clone Bot",
                value: payload.weatherClone.status === "healthy"
                    ? "LIVE"
        ...
        case "clone_coverage":
            return {
                ...
                footer: truncate(`${payload.weatherClone.executionAllowed ? "live" : "shadow"} • ${payload.weatherClone.candidateCount} cand`, 20),
                status: payload.weatherClone.status,
            };
        case "ops_alerts": {
...
function wrapCachedState(view, context) {
    if (!context.isCached)
        return view;
    return {
        ...view,
...
function buildPlaceholderTile(tileId, context) {
...
export function renderTileImage(tileId, payload, context) {
    const view = payload
