...
export function isTileId(value) {
    return typeof value === "string" && TILE_IDS.includes(value);
}
export function normalizeGlobalSettings(value) {
    const pollInterval = typeof value?.pollIntervalSec === "number"
    ...
    return {
        ...
        pollIntervalSec: Number.isFinite(pollInterval) && pollInterval > 0
...
export function normalizeTileSettings(value) {
    const tileId = isTileId(value?.tileId) ? value.tileId : DEFAULT_TILE_SETTINGS.tileId;
    const targetPath = typeof value?.targetPath === "string" && value.targetPath.trim().length > 0
